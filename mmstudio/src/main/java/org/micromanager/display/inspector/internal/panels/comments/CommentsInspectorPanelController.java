      ...
      implements DocumentListener
{
   ...
   public static AbstractInspectorPanelController create() {
      CommentsInspectorPanelController instance =
            new CommentsInspectorPanelController();
   ...
   private CommentsInspectorPanelController() {
      panel_.setLayout(new MigLayout(
            new LC().insets("4").gridGap("0", "0").fill()));
      ...
      JPanel summaryCommentsPanel = new JPanel(new MigLayout(
            new LC().insets("0").gridGap("0", "0").fill()));
      summaryCommentsPanel.add(new JLabel("Dataset Summary Comments:"),
            new CC().growX().wrap());
      summaryCommentsPanel.add(summaryScrollPane, new CC().grow().push());

      planeTextArea_ = new JTextArea();
      configureTextArea(planeTextArea_);
      JScrollPane planeScrollPane = new JScrollPane(planeTextArea_,
            JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
            JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
      JPanel planeCommentsPanel = new JPanel(new MigLayout(
            new LC().insets("0").gridGap("0", "0").fill()));
      planeCommentsPanel.add(new JLabel("Image Plane Comments:"),
            new CC().gapTop("4").growX().wrap());
      planeCommentsPanel.add(planeScrollPane, new CC().grow().push());

      JSplitPane splitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT,
            summaryCommentsPanel, planeCommentsPanel);
